...
export const SOMPI_PER_KASPA = 1_0000_0000n;
...
// https://github.com/kaspanet/rusty-kaspa/blob/v1.0.1/wallet/core/src/tx/mass.rs#L20
export const MINIMUM_RELAY_TRANSACTION_FEE = 1000n;
// https://github.com/kaspanet/rusty-kaspa/blob/v1.0.1/wallet/core/src/tx/mass.rs#L24
export const MAXIMUM_STANDARD_TRANSACTION_MASS = 100_000n;
// https://github.com/kaspanet/rusty-kaspa/blob/v1.0.1/consensus/core/src/constants.rs#L24
export const MAX_SOMPI = 29_000_000_000n * SOMPI_PER_KASPA;

/**
 * @typedef {import('./kaspa.js').TransactionOutpoint} TransactionOutpoint
 * @typedef {import('./kaspa.js').TransactionInput} TransactionInput
 * @typedef {import('./kaspa.js').TransactionOutput} TransactionOutput
 * @typedef {import('./kaspa.js').Transaction} Transaction
 */
...
/**
 * @param {TransactionInput} input - Transaction input-like object.
 * @returns {bigint} Serialized input size.
 */
...
/**
 * @param {TransactionOutput} output - Transaction output-like object.
 * @returns {bigint} Serialized output size.
 */
...
/**
 * @param {Transaction} tx - Transaction-like object.
 * @returns {bigint} Serialized transaction size.
 */
...
/**
 * @param {Transaction} tx - Transaction-like object.
 * @param {object} [params] - Mass parameters.
 * @param {bigint} [params.massPerTxByte] - Mass per serialized byte.
 * @param {bigint} [params.massPerScriptPubKeyByte] - Mass per script public key byte.
 * @param {bigint} [params.massPerSigOp] - Mass per signature operation.
 * @returns {bigint} Transaction compute mass.
 */
export function transactionComputeMass(tx, {
  ...
  for (const input of tx.inputs || []) {
    mass += massPerSigOp * BigInt(input.sigOpCount ?? 1);
  }
  return mass;
}

/**
 * @param {TransactionOutput|TransactionOutpoint} output - Output or UTXO-like object.
 * @returns {bigint} Plurality units.
 */
 ...
 * @param {Transaction} tx - Transaction-like object.
 * @param {object} [params] - Mass parameters.
 * @param {bigint} [params.storageMassParameter] - Storage mass multiplier.
 * @returns {bigint} Storage mass component.
 */
export function transactionStorageMass(tx, {
  ...
  const [insPlurality, harmonicIns, sumIns] = inputs
    .map((input) => {
      const plurality = utxoPlurality(input.utxo);
      return { plurality, amount: input.utxo.amount };
    })
    .reduce(
...
/**
 * @param {Transaction} tx - Transaction-like object.
 * @param {object} [params] - Mass parameters delegated to component calculators.
 * @param {bigint} [params.storageMassParameter] - Storage mass multiplier.
 * @param {bigint} [params.massPerTxByte] - Mass per serialized byte.
 * @param {bigint} [params.massPerScriptPubKeyByte] - Mass per script public key byte.
 * @param {bigint} [params.massPerSigOp] - Mass per signature operation.
 * @returns {bigint} Total transaction mass.
 */
...
export function isOutputDust(output) {
  if (output.amount <= 0n) return true;
  // why 148? why not 108 or even 170
  // https://github.com/kaspanet/rusty-kaspa/issues/750
  const totalSize = outputSize(output) + 148n;
  return output.amount * 1000n / (3n * totalSize) < MINIMUM_RELAY_TRANSACTION_FEE;
};

//export const MASS_PER_STANDARD_INPUT = inputSize({}) + MASS_PER_SIG_OP;
export const MASS_PER_STANDARD_INPUT = 1118n;
//export const MASS_PER_STANDARD_OUTPUT = outputSize({}) + 36n * MASS_PER_SCRIPT_PUB_KEY_BYTE;
export const MASS_PER_STANDARD_OUTPUT = 412n;
//export const MASS_PER_STANDARD_TRANSACTION = transactionSize({});
export const MASS_PER_STANDARD_TRANSACTION = 94n;

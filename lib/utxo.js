...
export const SOFT_DUST = 200_0000n;
...
export function select(utxos, outputs, options = {}) {
  const {
    maxWaste = HARD_DUST,
    feerate = 1n,
    maxFee = 100n * feerate * MAXIMUM_STANDARD_TRANSACTION_MASS,
  } = options;
  const best = {
    selected: undefined,
    fee: MAX_SOMPI,
  ...
  for (const alg of [branchAndBound, accumulative]) {
    for (const sortedUtxos of [
      ...
      const { fee, waste, selected, change } = alg(sortedUtxos, outputs, options);
      if (fee <= best.fee && waste < best.waste) {
        Object.assign(best, {
          selected,
          fee,
  ...
  if (best.fee > maxFee) {
    const i = utxos.map((u) => `${bytesToHex(u.utxo.transactionId)}#${u.utxo.index}`).join(',');

...
import { transactionMass } from './mass.js';
import {
  abytes,
  au16,
  au32,
...
export class Transaction {
  ...
  constructor({
    ...
    this.mass = mass !== undefined ? au64(mass, 'mass') : transactionMass(this);
  }
   ...
   * @param {Uint8Array} [auxRand] - 32-byte random data.
   * @returns {boolean} True if signing succeeds.
   */
  sign(privateKeyOrHDKey, auxRand) {
    ...
    for (let idx = 0; idx < this.inputs.length; idx++) {
      ...
      const signature = schnorr.sign(message, privKey, auxRand);

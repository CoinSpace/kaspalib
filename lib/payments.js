...
export const OutScript = {
   ...
   */
  encode({ version = MAX_SCRIPT_PUBLIC_KEY_VERSION, type, payload }) {
    if (version !== MAX_SCRIPT_PUBLIC_KEY_VERSION) {
      throw new Error(`OutScript/${type}: unsupported version ${version}`);
    }
    if (type === 'pk' && !isValidPubkey(payload, PUBKEY_TYPES.SCHNORR)) {
      throw new Error('OutScript/pk: wrong key');
    }
    if (type === 'pk-ecdsa' && !isValidPubkey(payload, PUBKEY_TYPES.ECDSA)) {
      throw new Error('OutScript/pk-ecdsa: wrong key');
    }
    if (type === 'sh'
      && (!isBytes(payload) || payload.length !== 32)) {
      throw new Error('OutScript/sh: wrong hash');
    }
    return {
      version,
      script: OutScriptCoder.encode({
        version,
        type,
        payload,
      }),
 ...
 * @returns {{ type: 'pk'; address: string; version: number; script: Uint8Array }} Payment.
 */
export function p2pk(pubkey, options) {
  return {
    type: 'pk',
    address: Address(options).encode({ type: 'pk', payload: pubkey }),
    ...OutScript.encode({
 ...
 * @param {object} [options] - Address options.
 * @returns {{ type: 'pk-ecdsa'; address: string; version: number; script: Uint8Array }} Payment.
 */
export function p2pkECDSA(pubkey, options) {
  return {
    type: 'pk-ecdsa',
    address: Address(options).encode({ type: 'pk-ecdsa', payload: pubkey }),
    ...OutScript.encode({
      version: MAX_SCRIPT_PUBLIC_KEY_VERSION,
      type: 'pk-ecdsa',
      payload: pubkey,
